 ...
 *      Author: yuchen
 *  It use Outputxbar2 to output idea 50Hz signal at pin 37 for compare
 *  Capture 1pps signal (pin 36) and then capture sample signal generated by EPWM (pin 40)
 *  Measure ADC in Pin 29 and Pin 28
 */
...
{
    GPIO_SetupPinOptions(GPIO_1PPS, GPIO_INPUT, GPIO_ASYNC);

    EALLOW;
#if USEECAP == 1
    InputXbarRegs.INPUT7SELECT = GPIO_EPWM;         // Set eCAP1 source to GPIO 0, pin 40
    InputXbarRegs.INPUT8SELECT = 0;         // Set eCAP2 source to GPIO 0
    InputXbarRegs.INPUT9SELECT = 0;         // Set eCAP3 source to GPIO 4
    PieVectTable.ECAP1_INT = &ecap0_isr;
    PieCtrlRegs.PIEIER4.bit.INTx1 = 1;      //PIEIER4.1 is ECAP1
#elif USEECAP == 4
    InputXbarRegs.INPUT10SELECT = GPIO_EPWM;         // Set eCAP4 source to GPIO 4, pin 36
    InputXbarRegs.INPUT11SELECT = 0;         // Set eCAP5 source to GPIO 0
    InputXbarRegs.INPUT12SELECT = 0;
    ...
    /*CONT_ONESHT:1 =0;    0 Continuous or one-shot, choose Continuous
      ...
      SWSYNC:1   =0        8 SW forced counter sync
      CAP_APWM:1 =1        9 CAP/APWM operating mode select
      APWMPOL:1  =0        10 APWM output polarity select
      rsvd1:5;             15:11 Reserved*/

    ecap1->ECCLR.all = 0xFFFF;      // Clear all CAP __interrupt flags
    ecap1->ECEINT.all = 0x0000;    // disable all interrupt bit
    ecap1->CAP1 = CPU_CLOCK / POWER_CYCLE;
    ecap1->CAP2 = CPU_CLOCK / POWER_CYCLE / 2;
    ecap1->ECCTL1.bit.FREE_SOFT = 0x00;      // Emulation mode, stop
    ecap1->ECCTL2.bit.TSCTRSTOP = 1;     // Start Counter
#if USEECAP == 1
    OutputXbarRegs.OUTPUT2MUX0TO15CFG.bit.MUX2 = 3; // Select ecap1 out on Mux0, see hardware_reg table 8.3
    OutputXbarRegs.OUTPUT2MUXENABLE.bit.MUX2 = 1;  // Enable MUX0 for ECAP1.OUT
#else
    OutputXbarRegs.OUTPUT2MUX0TO15CFG.bit.MUX8 = 3; // Select ecap1 OUT on Mux0, see hardware_reg table 8.3
    OutputXbarRegs.OUTPUT2MUXENABLE.bit.MUX8 = 1;  // Enable MUX0 for ECAP1.OUT
#endif

#if GPIO_50HZ == 3
    GpioCtrlRegs.GPAPUD.bit.GPIO3 = 1;     //disable PULL up
    GpioCtrlRegs.GPAGMUX1.bit.GPIO3 = 1;
    GpioCtrlRegs.GPAMUX1.bit.GPIO3 = 1;    // Select OUTPUTXBAR2 on GPIO3,pin 37, see pin mux table 4.3
#elif GPIO_50HZ == 25
    GpioCtrlRegs.GPAPUD.bit.GPIO25 = 1;
    GpioCtrlRegs.GPAGMUX1.bit.GPIO25 = 0;
    GpioCtrlRegs.GPAMUX1.bit.GPIO25 = 1;    // Select OUTPUTXBAR2 on GPIO5,pin 35, see pin mux table 4.3
#endif
#endif
...
static void epwm_init()
{
    ...
    epwm->CMPA.bit.CMPA = DEFAULT_SAMPLE_PERIOD / 2-1;      //set to 0 when equal to CMPA
}
...
// bit 5-0       000000: BURSTTRIGSEL, 00=software only (don't care)
    ...
    adc->ADCSOC0CTL.bit.CHSEL  = 2; //Choose ADCIN2, for ADCINA2, it is pin 29, for ADCINA3 it is pin 26
    adc->ADCSOC1CTL.bit.CHSEL  = 2;
    adc->ADCSOC2CTL.bit.CHSEL  = 3;
    adc->ADCSOC3CTL.bit.CHSEL  = 3;
...
void ADCSync1PPS::push_dma_sync_tbl(int sample_period, uint64_t sample_idx, uint64_t tick)
{
    ...
    printf("dma sample=%llu, tick=%llu, period=%d\n\r", sample_idx, tick, sample_period);
}
...
void ADCSync1PPS::update_tick_ps()
{
    ...
    if (new_tick_ps <= tick_ps + 3 && new_tick_ps >= tick_ps - 3)
        tick_ps = new_tick_ps;
    else
        tick_ps = (tick_ps * 3 + new_tick_ps) / 4; //IIR filter
    uint32_t sample_period = epwm->TBPRD + 1;
    uint32_t low_th = sample_period  * SAMPLE_RATE - SAMPLE_RATE * 7 / 8;
    uint32_t high_th = sample_period  * SAMPLE_RATE + SAMPLE_RATE * 7 / 8;
    if (tick_ps <= low_th) {
        epwm->TBPRD = sample_period - 2; //TBPRD--
        set_cap_state(CAPTURE_EPWM);
    }
    if (tick_ps >= high_th) {
        epwm->TBPRD = sample_period; //TBPRD++
        set_cap_state(CAPTURE_EPWM);
    }
}
...
void ADCSync1PPS::set_cap_state(uint16_t cap_state)
{
    ...
    case CAPTURE_EPWM:
#if USEECAP == 1
        InputXbarRegs.INPUT7SELECT = GPIO_EPWM;
#elif USEECAP == 4
        InputXbarRegs.INPUT10SELECT = GPIO_EPWM;
#endif
    ...
    case CAPTURE_50HZ:
#if USEECAP == 1
        InputXbarRegs.INPUT7SELECT = GPIO_50HZ;
#elif USEECAP == 4
        InputXbarRegs.INPUT10SELECT = GPIO_50HZ;
#endif
        ecap0->ECEINT.bit.CEVT1 = 1;
        break;
    default:
    ...
    ecap0->ECCLR.all = 0x2; //clear CAP1 interrupt
...
void ADCSync1PPS::new_cap_evt()
{
    EALLOW;
    uint16_t ecflg = ecap0->ECFLG.all;
    uint32_t lowcap1 = ecap0->CAP1;
    cap1 = tick_h32;
    if (ecflg & 0x20) { //overflow happen
        if (lowcap1 > 0x70000000)
            cap1--;
    }
    cap1 = (cap1 << 32) + lowcap1;
    switch (sync_state) {
    case SYNC_ENTER:
        break;
    ...
    case SYNC_UTC1:
    case SYNC_UTC2:
            receive_1pps_epwm = capture_state;
            event_pending |= ADCSYNC_EVENT_MASK;
#ifdef GPIO_50HZ
            if (capture_state == CAPTURE_50HZ) {
                int32_t delay = cap1 - utc_sync_tbl[UTCSYNC_SIZE-1].tick;
                int32_t first_cycle;
                if (delay > tick_pc / 2) {
                    tick_pc = tick_ps / POWER_CYCLE; //at this time tick_ps is already updated
                    first_cycle = tick_ps - delay - tick_pc * (POWER_CYCLE - 2); //delay is near to tick_pc
                } else {
                    tick_pc = tick_ps / POWER_CYCLE; //at this time tick_ps is already updated
                    first_cycle = tick_ps - delay - tick_pc * (POWER_CYCLE - 1); //delay is near to 0
                }
                ecap1->CAP1 = first_cycle - 1;
                ecap1->CAP2 = first_cycle/2;
                ecap1->CAP3 = tick_pc - 1;
                ecap1->CAP4 = tick_pc/2;
            }
#endif
        break;
    case SYNC_INIT2:
...
void ADCSync1PPS::init()
{
    ...
    record_utc = 0;
    tick_ps = CPU_CLOCK;
    tick_pc = CPU_CLOCK / POWER_CYCLE;
    receive_1pps_epwm = CAPTURE_NONE;
    adc_init();
    compute_vi.init();
    init_xbar_gpio();
    ecap_init();
...
void ADCSync1PPS::process_event()
{
    ...
    while (!finish) {
        finish = true;
        if (receive_1pps_epwm == CAPTURE_1PPS) { //receive 1pps
            receive_1pps_epwm = CAPTURE_NONE;
...
#ifdef GPIO_50HZ
                        if (capture_state != CAPTURE_EPWM) {//if no need to capture epwm
                            set_cap_state(CAPTURE_50HZ); //capture ideal 50Hz
                        }
#endif
                    }
                }
        ...
        if (receive_1pps_epwm == CAPTURE_EPWM) {
            receive_1pps_epwm = CAPTURE_NONE;
            uint64_t tick = cap1;
#ifdef GPIO_50HZ
            set_cap_state(CAPTURE_50HZ); //capture ideal 50Hz
            ...
            sample = tick2sample(tick);
            Assert(sample!=0);
            push_dma_sync_tbl(epwm->TBPRD + 1, sample, tick);
        }

#ifdef GPIO_50HZ
        if  (receive_1pps_epwm == CAPTURE_50HZ) {
            receive_1pps_epwm = CAPTURE_NONE;
            uint64_t tick = cap1;
            int32_t delay = cap1 - utc_sync_tbl[UTCSYNC_SIZE-1].tick;
            if (delay > tick_pc / 2)
                delay -= tick_pc;
            printf("1pps delta = %ld\n\r", delay);
            set_cap_state(CAPTURE_1PPS);
        }
#endif
        ...
        if (sync_state == SYNC_UTC1 || sync_state == SYNC_UTC2) {
            ...
            if (ret != WRONG_ESTIMATE) {
                ...
                if (ret == COMPUTEVI_INTIME) {
                    ...
                    if (record_utc % 500 == 0 && keep_display) {
                        printf("(%d,%d.%2d),(%d,%d.%2d)",
                               vibuf.vi[0][0], vibuf.vi[0][1] / 100, abs(vibuf.vi[0][1] % 100),
                               vibuf.vi[2][0], vibuf.vi[2][1] / 100, abs(vibuf.vi[2][1] % 100));
                        if (record_utc % 1000 == 0)
                        printf("pa=%ld, tick_ps=%ld, T=%lld\n\r", phase_adjust, tick_ps, get_tick());
                    }
